...
- **Global scope**: `C:/code/manager-class/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/code/manager-class/.claude/notes/`)
...
2. Glob `C:/code/manager-class/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/code/manager-class/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/code/manager-class/.claude/notes/` — works in any directory
