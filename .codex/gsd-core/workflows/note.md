...
- **Global scope**: `C:/code/manager-class/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/code/manager-class/.codex/notes/`)
...
2. Glob `C:/code/manager-class/.codex/notes/*.md` (if directory exists) — global notes
...
Global (C:/code/manager-class/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/code/manager-class/.codex/notes/` — works in any directory
